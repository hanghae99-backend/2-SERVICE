...
class ChargeBalanceRequestUnitTest : BehaviorSpec({
    
    given("ChargeBalanceRequest DTO") {
        When("정상적인 값으로 생성할 때") {
            Then("정상적으로 생성된다") {
                val userId = 1L
            ...
            Then("경계값으로 생성할 때 정상 처리된다") {
                ...
                val request = ChargeBalanceRequest(userId, minAmount)
            ...
            Then("큰 금액으로 생성할 때 정상 처리된다") {
                ...
                val request = ChargeBalanceRequest(userId, largeAmount)
        ...
        When("유효하지 않은 값으로 생성할 때") {
            Then("0원으로 생성 시 예외가 발생한다") {
                ...
                shouldThrow<IllegalArgumentException> {
                    ChargeBalanceRequest(userId, zeroAmount)
                }
            }
            ...
            Then("음수 금액으로 생성 시 예외가 발생한다") {
                ...
                shouldThrow<IllegalArgumentException> {
                    ChargeBalanceRequest(userId, negativeAmount)
                }
            }
        ...
        When("같은 값으로 두 개의 request를 생성할 때") {
            Then("동등성 비교가 정상 처리된다") {
                ...
                val request1 = ChargeBalanceRequest(userId, amount)
                val request2 = ChargeBalanceRequest(userId, amount)
