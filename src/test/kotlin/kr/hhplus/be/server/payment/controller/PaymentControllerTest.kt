    ...
    describe("POST /api/v1/payments") {
        context("유효한 결제 요청이 들어올 때") {
            it("결제를 처리하고 201 상태코드를 반환해야 한다") {
                ...
                every { paymentService.processPayment(userId, reservationId, token) } returns paymentDto
        ...
        context("잘못된 형식의 요청이 들어올 때") {
            it("400 상태코드를 반환해야 한다") {
                ...
                every { paymentService.processPayment(invalidRequest.userId, invalidRequest.reservationId, invalidRequest.token) } throws IllegalArgumentException("잘못된 요청입니다")
                // when & then
                mockMvc.perform(
                    post("/api/v1/payments")
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        context("결제 처리 실패 시") {
            it("400 상태코드를 반환해야 한다") {
                ...
                every { paymentService.processPayment(userId, reservationId, token) } throws
    ...
    describe("GET /api/v1/payments/{paymentId}") {
        ...
        context("존재하지 않는 결제 ID로 조회할 때") {
            it("404 상태코드를 반환해야 한다") {
                ...
                every { paymentService.getPaymentById(paymentId) } throws
