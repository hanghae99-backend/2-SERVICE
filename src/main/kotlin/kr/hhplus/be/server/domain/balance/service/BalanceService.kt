...
class BalanceService(
    ...
    private fun chargeBalanceInternal(userId: Long, amount: BigDecimal): Point {
        ...
        val currentPoint = pointRepository.findByUserId(userId)
            ?: Point.Companion.create(userId, BigDecimal.ZERO)
        ...
        val history = PointHistory.Companion.charge(userId, amount, chargeType, "포인트 충전")
    ...
    fun deductBalanceInternal(userId: Long, amount: BigDecimal): Point {
        ...
        val history = PointHistory.Companion.use(userId, amount, useType, "포인트 사용")
        pointHistoryRepository.save(history)
